...
bool text_open(char *filename)
{
	...
	if(verbose) printf("Attempting to open file %s...\n", filename);
	...
	if(verbose) printf("File %s opened, %i bytes.\n", filename, size);
	...
	if(text_src) {
		if(verbose) printf("text_src exists, freeing...");
		free(text_src);
		if(verbose) printf(" done.\n");
	}

	if(verbose) printf("Allocating buffer for text_src...");
	...
	if(verbose)printf("freeing text...");
	free(text);
	if(verbose)printf(" done.\n");
...
{
	if(verbose)printf("text_close called. Working its magic...\n");
	if(text_src)
	{
		if(verbose)printf("Attempting to free text_src...");
		free(text_src);
		if(verbose)printf(" done.\n Nullifying text_src...");
		text_src=NULL;
		if(verbose)printf(" done.\n Zeroing text_size...");
